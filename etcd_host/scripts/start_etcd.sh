    ...
    if [ -n "$VHOSTS" ]; then
        while read -r line; do
            ...
            if [[ "$line" == *":"* ]]; then
                ...
                if [ "${#vhost[@]}" -lt 3 ]; then
                    echo "Vhost line incorrect. Should be external_port:internal_port:hostname1,hostname2[:etcd_prefix]";
                else
                    ...
                    etcdctl -C "$ETCD" mkdir "/$prefix/$IP_clean" --ttl 30 2>&1 1> /dev/null # Redirect stderr to stdout then stdout to null
                    etcdctl -C "$ETCD" set "/$prefix/$IP_clean/ip" "$IP" 2>&1 1> /dev/null   # so only stderr will be printed in the log
                    etcdctl -C "$ETCD" set "/$prefix/$IP_clean/config/$vhost_number/external_port" "$external_port" 2>&1 1> /dev/null
                    etcdctl -C "$ETCD" set "/$prefix/$IP_clean/config/$vhost_number/internal_port" "$internal_port" 2>&1 1> /dev/null
                    etcdctl -C "$ETCD" set "/$prefix/$IP_clean/config/$vhost_number/hostnames" "$hostnames" 2>&1 1> /dev/null
                    i=$((i+1))
                fi
            else
                # VHOST parameter (e.g. VHOST_EXTERNAL_PORT=80)
                # Not supporting prefix defined in compact declaration. We always use /hosts/
                ...
                value="${vhost[1]}"
                etcdctl -C "$ETCD" mkdir "/$prefix/$IP_clean" --ttl 30 > /dev/null 2>&1 # Do not print anything
                etcdctl -C "$ETCD" set "/$prefix/$IP_clean/ip" "$IP" 2>&1 1> /dev/null   # so only stderr will be printed in the log
                etcdctl -C "$ETCD" set "/$prefix/$IP_clean/config/$vhost_number/$option" "$value" 2>&1 1> /dev/null
            fi
        done <<< "$VHOSTS"
        ...
        while true; do
            ...
            if ! etcdctl -C "$ETCD" updatedir "/$prefix/$IP_clean" --ttl 30; then
